...
const procedure = require("../procedures/datamanagement");
...
Router.post("/saveuserpermissions", helpers.middle, async function (req, res) {
  res.header("Access-Control-Allow-Origin", process.env.allowed_sites);
  res.header(
    "Access-Control-Allow-Headers",
    "Origin, X-Requested-With, Content-Type, Accept, Authorization"
  ...
  try {
    ...
    if (data.root.users.login_password != undefined) {
      const salt = bcrypt.genSaltSync(10);
      console.log("salt", salt);
      var password = bcrypt.hashSync(data.root.users.login_password, salt);
      console.log(password);
      data.root.users.login_password = password;
    }

    console.log("final_data", data.root.users);
    var result = await dbcontext.CallSP(
      "com.save_user_permissions",
